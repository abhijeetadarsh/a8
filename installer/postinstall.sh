...
# --- the login screen -------------------------------------------------------
# LightDM starts X for you, so booting lands on a graphical login rather than a
# TTY where you type `startx`. The GTK greeter is the one that can be themed
# from what this repo already generates: it reads a plain GTK stylesheet, and
# theme_engine writes one.
#
# Both are in the official repos, which matters - a display manager is not
# something to have depending on an AUR build that can break on an upgrade and
# leave you unable to log in.
PKGS_LOGIN=( lightdm lightdm-gtk-greeter )
...
GREETER_TAG="# written by postinstall.sh"

do_lightdm() {
    step "login screen"

    if ! command -v lightdm >/dev/null; then
        note "lightdm is not installed, skipping the login screen"
        return 0
    fi

    # The greeter runs as the lightdm user, before anyone has logged in, so it
    # cannot read anything under /home - not the palette, not the wallpaper.
    # This directory is the handover point: root creates it once, here, owned
    # by you, and theme_init.sh copies into it afterwards with no privileges at
    # all. The alternative is a NOPASSWD sudo rule that fires on every
    # wallpaper change, which is a far worse trade for a background image.
    sudo install -d -o "$USER" -g "$(id -gn)" -m 755 "$GREETER_DIR" || {
        warn "could not create $GREETER_DIR - the greeter will use a flat colour"
        return 1
    }

    # GTK looks for a user stylesheet in the *greeter's* home. A symlink out to
    # the handover directory means the greeter picks up a new palette with no
    # further root involvement.
    sudo install -d -o lightdm -g lightdm -m 755 \
        /var/lib/lightdm/.config /var/lib/lightdm/.config/gtk-3.0 2>/dev/null
    sudo ln -sfn "$GREETER_DIR/gtk.css" /var/lib/lightdm/.config/gtk-3.0/gtk.css

    if [[ -f "$GREETER_CONF" ]] && ! grep -qF "$GREETER_TAG" "$GREETER_CONF"; then
        sudo cp -n "$GREETER_CONF" "$GREETER_CONF.pacorig" 2>/dev/null
    fi

    sudo tee "$GREETER_CONF" >/dev/null <<EOF
$GREETER_TAG
#
# The background and the GTK stylesheet are both regenerated by theme_init.sh
# on every wallpaper change, into $GREETER_DIR. Nothing here needs
# editing when the theme changes.
[greeter]
background = $GREETER_DIR/background.png
theme-name = Adwaita-dark
icon-theme-name = Papirus-Dark
cursor-theme-name = Adwaita
font-name = Fantasque Sans Mono 12
xft-antialias = true
xft-hintstyle = slight
# The per-user background would override the one above with whatever the
# account has set, which is not the themed image.
...
EOF
    ...
    if sudo systemctl enable lightdm.service >/dev/null 2>&1; then
        ok "lightdm enabled - the next boot lands on a graphical login"
    else
        warn "could not enable lightdm.service"
    fi
    return 0
}
...
do_xinitrc() {
    ...
    # The session environment lives in ~/.xprofile, not in ~/.xinitrc.
    #
    # There are two ways into this desktop and they share almost nothing.
    # `startx` runs ~/.xinitrc. LightDM does not: it runs /etc/lightdm/Xsession,
    # which sources ~/.xprofile and merges ~/.Xresources itself, then execs the
    # session from /usr/share/xsessions/i3.desktop - a bare `Exec=i3`. So
    # anything exported only from .xinitrc is silently absent under the display
    # manager, and Qt apps come up unthemed, Java apps come up blank, and
    # $mod+Shift+w loses its source. One file, sourced by both, avoids that.
    # WALLPAPER_SOURCE is the one line in the generated file that is meant to be
    # edited, so a re-run has to carry the existing choice forward. Rewriting it
    # back to the default every time would silently undo it. Checked in the old
    # .xinitrc too, which is where it lived before .xprofile existed.
    local wp_source="" f
    for f in "$HOME/.xprofile" "$HOME/.xinitrc"; do
        [[ -f "$f" ]] || continue
        wp_source="$(sed -n 's/^[[:space:]]*export[[:space:]]\+WALLPAPER_SOURCE=\([A-Za-z_.]*\).*/\1/p' "$f" | tail -1)"
        [[ -n "$wp_source" ]] && break
    done
    if [[ -n "$wp_source" && "$wp_source" != my_collection ]]; then
        note "keeping WALLPAPER_SOURCE=$wp_source"
    fi
    wp_source="${wp_source:-my_collection}"

    if [[ -e "$HOME/.xprofile" && ! -L "$HOME/.xprofile" ]] &&
       ! grep -qF "$XPROFILE_TAG" "$HOME/.xprofile"; then
        note "~/.xprofile already exists and is not ours, leaving it alone"
...
$XPROFILE_TAG
#
# Sourced by /etc/lightdm/Xsession before it starts your session, and by
# ~/.xinitrc when you use startx instead. Put session-wide environment here so
# both routes agree; anything that has to *run* belongs in the i3 config.
...
# Where \$mod+Shift+w gets a new wallpaper from:
#
...
#
# Edit this line and log out and back in to change it; a postinstall re-run
# keeps whatever it says. Logging in never picks a new wallpaper regardless -
# i3 runs theme_init.sh --reload, which puts back the one already in use.
export WALLPAPER_SOURCE=$wp_source
EOF
        ok "wrote ~/.xprofile (session environment, shared by startx and lightdm)"
    fi
    ...
    if [[ -e "$HOME/.xinitrc" && ! -L "$HOME/.xinitrc" ]] &&
       ! grep -qF "$XINITRC_TAG" "$HOME/.xinitrc"; then
        ...
        note "for the same environment lightdm gets it needs: . ~/.xprofile"
        return 0
    fi
...
$XINITRC_TAG
#
# LightDM does not read this file at all - see ~/.xprofile, which both routes
# share. Keep this one to the parts startx has to do for itself.

[ -f ~/.Xresources ] && xrdb -merge ~/.Xresources
[ -f ~/.cache/theme/colors.Xresources ] && xrdb -merge ~/.cache/theme/colors.Xresources

# The session environment. LightDM sources this itself, from Xsession.
[ -f ~/.xprofile ] && . ~/.xprofile

exec i3
EOF
    chmod +x "$HOME/.xinitrc"
    ok "wrote ~/.xinitrc (startx -> i3, session env from ~/.xprofile)"
}
...
do_theme() {
    ...
    # The lock screen and the login greeter share one background image, sized
    # to a single monitor. That size normally comes from xrandr - but the first
    # run of this script happens before X has ever started, and the *greeter*
    # is needed before any i3 session exists to generate one. So write it at a
    # sensible default now and let the first theme_init.sh replace it at the
    # real resolution, exactly like the monitors.conf placeholder below.
    local lock_size="1920x1080"
    if [[ -n "${DISPLAY:-}" ]] && command -v xrandr >/dev/null; then
        local live
        live=$(xrandr --query 2>/dev/null | awk '
            / connected/ { for (i = 3; i <= NF; i++)
                if ($i ~ /^[0-9]+x[0-9]+\+-?[0-9]+\+-?[0-9]+$/) {
                    split($i, g, "+"); split(g[1], r, "x")
                    if (r[1] * r[2] > best) { best = r[1] * r[2]; w = r[1]; h = r[2] }
                    break } }
            END { if (w) print w "x" h }')
        [[ "$live" =~ ^[0-9]+x[0-9]+$ ]] && lock_size="$live"
    fi

    mkdir -p "$HOME/.cache/theme"
    if python "$engine" "$wall" -o "$HOME/.cache/theme" \
            --screen-image "$lock_size:$HOME/.cache/theme/lock.png" \
            --screen-image-dim 0.55 >/dev/null 2>&1; then
        printf '%s\n' "$wall" > "$HOME/.cache/theme/wallpaper"
        ok "palette generated - i3, polybar, rofi, kitty, dunst, gtk, nvim and the prompt all read it"

        # The greeter runs before login, so its background cannot wait for the
        # first theme_init.sh. do_lightdm created this directory owned by us.
        if [[ -w "$GREETER_DIR" && -f "$HOME/.cache/theme/lock.png" ]]; then
            cp -f "$HOME/.cache/theme/lock.png" "$GREETER_DIR/background.png" &&
                ok "login screen themed from the same wallpaper"
        fi
    else
